...
import es.gobcan.istac.statistical.operations.roadmap.core.repository.PersistenceAuditEventRepository;
import es.gobcan.istac.statistical.operations.roadmap.core.service.AuditEventService;
import es.gobcan.istac.statistical.operations.roadmap.core.service.impl.AuditEventServiceImpl;
import es.gobcan.istac.statistical.operations.roadmap.internal.rest.resources.AuditResource;
...
public class AuditResourceIntTest {
    ...
    public void getAllAudits() throws Exception {
        ...
        // Get all the audits
        restAuditMockMvc.perform(get(ENDPOINT_URL)).andExpect(status().isOk()).andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
    ...
    public void getAudit() throws Exception {
        ...
        // Get the audit
        restAuditMockMvc.perform(get(ENDPOINT_URL + "/{id}", auditEvent.getId())).andExpect(status().isOk()).andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
    ...
    public void getAuditsByDate() throws Exception {
        ...
        // Get the audit
        restAuditMockMvc.perform(get(ENDPOINT_URL + "?fromDate=" + fromDate + "&toDate=" + toDate)).andExpect(status().isOk()).andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
                .andExpect(jsonPath("$.[*].principal").value(hasItem(SAMPLE_PRINCIPAL)));
    }
    ...
    public void getNonExistingAudit() throws Exception {
        // Get the audit
        restAuditMockMvc.perform(get(ENDPOINT_URL + "/{id}", Long.MAX_VALUE)).andExpect(status().isNotFound());
    }
}
